...
Roteador leve de links para KDE Plasma 6: intercepta a abertura de URLs e
decide, por regras configuráveis, em qual navegador cada uma deve abrir —
podendo reescrever parâmetros da URL no processo (ex: `meet.google.com` →
Chrome com `&authuser=3`; `discord.com` → Firefox). Ao rotear uma URL que
casou com alguma regra, mostra uma notificação do sistema avisando pra onde
foi.
...
Toda tag `v*` dispara `.github/workflows/flatpak-release.yml`, que builda o
Flatpak e anexa o bundle `.flatpak` a uma GitHub Release. Isso não publica no
Flathub (é um passo manual separado, via PR em flathub/flathub).
